...
class GenerateHexagon():
    ...
    def findParent(self):
        ...
        data = json.load(fp)
        for i in data:
            k = self.findNearestCell(i.get('center'))
            if k in parent:
                parent[k].append(i)
            else:
                parent[k] = [i]
    ...
    def findAddress(self, point):
        geodata = Nominatim(user_agent='dillu').reverse(point)
        if geodata:
    ...
    def saveInDB(self):
        ...
        for i in self.result:
            cell = {
                ...
                'address': self.findAddress(self.result[i][0]),
                'inside': parent.get(i, [])
            }
            res.append(cell)
            # time.sleep(60)
